...
由于分类任务和定位不是严格正相关的，上述方法只是提高了分类的准确性，但是没有提供更好的定位性能。为了提升检测框的精度，将伪标签的前景分数大于 0.5 的目标框向四
周随机抖动，再通过教师网络预测这些抖动后的目标框区域，将重新预测的结果和抖动的结果对比，如果两个之间的差异越小，说明这个候选框越可靠，这个损失被定义为：
...
部分标签数据：使用1%、5%和 10%的数据作为有标注数据，其余作为无标注数据进行训练
